...
RPI_FIRMWARE_CUSTOM_FILES = \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_BOOTCODE_BIN), bootcode.bin) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI), start.elf fixup.dat) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI_X), start_x.elf fixup_x.dat) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI_CD), start_cd.elf fixup_cd.dat) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI_DB), start_db.elf fixup_db.dat) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI4), start4.elf fixup4.dat) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI4_X), start4x.elf fixup4x.dat) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI4_CD), start4cd.elf fixup4cd.dat) \
	$(if $(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_VARIANT_PI4_DB), start4db.elf fixup4db.dat)

define RPI_FIRMWARE_CUSTOM_INSTALL_BIN
	$(foreach f,$(RPI_FIRMWARE_CUSTOM_FILES), \
		$(INSTALL) -D -m 0644 $(@D)/boot/$(f) $(BINARIES_DIR)/rpi-firmware/$(f)
	)
endef

RPI_FIRMWARE_CUSTOM_CONFIG_FILE = $(call qstrip,$(BR2_PACKAGE_RPI_FIRMWARE_CUSTOM_CONFIG_FILE))
ifneq ($(RPI_FIRMWARE_CUSTOM_CONFIG_FILE),)
define RPI_FIRMWARE_CUSTOM_INSTALL_CONFIG
	$(INSTALL) -D -m 0644 $(RPI_FIRMWARE_CUSTOM_CONFIG_FILE) \
...
define RPI_FIRMWARE_CUSTOM_INSTALL_TARGET_LIB
...
define RPI_FIRMWARE_CUSTOM_INSTALL_TARGET_CMDS
	...
	$(RPI_FIRMWARE_CUSTOM_INSTALL_TARGET_LIB)
endef
endif # INSTALL_VCDBG

define RPI_FIRMWARE_CUSTOM_INSTALL_IMAGES_CMDS
	$(INSTALL) -D -m 0644 $(BR2_EXTERNAL_EDGEOS_PATH)/package/rpi-firmware-custom/cmdline.txt $(BINARIES_DIR)/rpi-firmware/cmdline.txt
	$(RPI_FIRMWARE_CUSTOM_INSTALL_BIN)
	$(RPI_FIRMWARE_CUSTOM_INSTALL_CONFIG)
